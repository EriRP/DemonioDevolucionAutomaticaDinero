...
using Operativa.ComponenteSQL.Utiles;
...
namespace Operativa.ComponenteSQL.Conexion.MYSQL
{
    public class MYSQL
    {
        ...
        //Fecha Modificación:
        //Cambios Realizados:
        public DataTable MYSQLSelect(string tipoconexion, string query)
        {
            ...
            try
            {
                //Si la conexión no se establece reintentamos 2 veces esperando 2 segundos entre cada intento
                while (johexitbandera < 1 && johexitcant < 3)
                {
        ...
        /*public DataTable MYSQLSelect_Masivo(string tipoconexion, string query)
        {
        ...
        public bool MYSQLInsert(string tipoconexion, string tablaInsert, Dictionary<string, object> Dict_Insert, string metodo, string contrato)
        {
            ...
            string Usuario = "";
            //if (!Util.IsNullOrEmpty(System.Web.HttpContext.Current.Session["usuario"]))
            //{
            //    Usuario = System.Web.HttpContext.Current.Session["usuario"].ToString().ToUpper();
            //}
            /*Fin Usuario*/
            ...
            try
            {
                ...
                tmpMySqlInsertC = MYSQLSelect(tipoconexion, "select * from " + tablaInsert + " Limit 1");
                    ...
                    if (!Util.IsNullOrEmpty(Campos2[i, 1]))//Siempre y cuando el valor sea diferente a vacio
                    {
                        ...
                        switch (Campos2[i, 2])
                        {
                            ...
                            case "FLOAT":
                                ...
                                if (Decimal.TryParse(Campos2[i, 1], style, culture, out number))//Si es TRUE EL FORMATO DEL NUMERO ES VALIDO
                                {//Correcto es numerico
                                    numerico = true;// Bandera para no concantenar el valor a un string 
                                    Valorx = number + "";
                            ...
                            case "DATETIME":
                                ...
                                string Date = Campos2[i, 1];//Almacena la fecha
                                ...
                                "d-M-yyyy HH:mm:ss",
                                //Nuevo Agregado
                                "MM/dd/yyyy", "MM-dd-yyyy",  "MM/dd/yyyy hh:mm:ss tt", "MM-dd-yyyy HH:mm:ss", "MM/dd/yyyy HH:mm:ss","MM-dd-yyyy hh:mm:ss tt"
                                ...
                                else
                                {
                                    ...
                                    /*Reintenta yyyy-MM-dd*/

                                    Date = Util.SUBSTR(Date, 0, 10);//Dejamos solo fecha
                                    ...
                                    if (DateTime.TryParseExact(Date, formats2, System.Globalization.CultureInfo.InvariantCulture, DateTimeStyles.None, out FECHA_FORMAT))
                                    {//si EXISTE UN FORMATO VALIDO 
                                     //Si la fecha es valida entonces se remplaza el formato a yyyy/MM/dd para insertar en ADV
                                        string Fecha_new = FECHA_FORMAT.ToString("yyyy/MM/dd HH:mm:ss");
                ...
                try
                {
                    ...
                    if (!mySqlSelectconex.Contains("error") && !string.IsNullOrEmpty(mySqlSelectconex) && !string.IsNullOrEmpty(sentencia_sql))
                    {
                        ...
                        banderaTransaccion = conexion.Executes_Mysql(sentencia_sql, mySqlSelectconex);
        ...
        public bool MYSQLUpdate(string tipoconexion, string tablaUpdate, Dictionary<string, object> oDict_, Trictionary<string, object, string[]> oDict2_, string metodo, string contrato)
        {
            ...
            {
                DataTable tmpMySqlInsertC = new DataTable();
                tmpMySqlInsertC = MYSQLSelect(tipoconexion, "select * from " + tablaUpdate + " Limit 1");
                        ...
                        switch (Campos2[i, 2])
                        {
                            ...
                            case "FLOAT":
                                ...
                                {//Correcto es numerico
                                    ...
                                    Valorx = number + "";
                                    //Valorx = Valorx.Replace(",", ".");
                                    sentencia_bit = sentencia_bit + Campos2[i, 0] + " : " + number + " ";
                            ...
                            case "DATETIME":
                                ...
                                /*Validacion de tt, C# no admite el formato con puntos! Limpiamos la fecha*/
                                //var Date = "";//Almacena la fecha
        ...
        public bool MYSQLDelete(string tipoconexion, string tablaDelete, Trictionary<string, object, string[]> oDict, string metodo, string contrato)
        {
            ...
            {
                DataTable tmpMySqlInsertC = new DataTable();
                tmpMySqlInsertC = MYSQLSelect(tipoconexion, "select * from " + tablaDelete + " Limit 1");
                ...
                for (int i = 0; i < long2; i++)
                {
                    ...
                    switch (Campos2[i, 2])
                    {
                        ...
                        case "DATETIME":
                            ...
                            /*Validacion de tt, C# no admite el formato con puntos! Limpiamos Fecha*/
                            //var Date = "";//Almacena la fecha
        ...
        public bool MYSQLDirecto(string tipoconexion, string tabla, string query, string metodo, string contrato)
        {
            ...
            try
            {
                ...
                {
                    //AQUI HACEMOS LA INSERCCION DE MYSQL
                    banderaTransaccion = conexion.Executes_Mysql(query, mySqlSelectconex);
        ...
        //Fecha Modificación:
        //Cambios Realizados:
        public bool MYSQLMASIVO(string tipoconexion, string tablaInsert, DataTable Dict_Insert_Masivo, string metodo)
        {
            //newCulture = new CultureInfo("en-US");
            //CultureInfo.DefaultThreadCurrentCulture = newCulture;
            //CultureInfo myCIclone = (CultureInfo)newCulture.Clone();
                ...
                foreach (DataRow row in Dict_Insert_Masivo.Rows)
                {
                    ...
                    if (cont_values == 800)//Inserta en paquetes de 800
                    {
                        ...
                        try
                        {
                            ...
                            if (/*!banderaTransaccion &&*/ !tablaInsert.ToLower().Contains("bitacora_componente_sql"))
                            {
                                //Si se cae el query insertamos en bitacora
                                //Mientras sea diferente de la bitacora, para no enciclar


                                Util.Bitacora_ComponenteSQL(Usuario, "INSERT MASIVO EN LA TABLA: " + tablaInsert, banderaTransaccion, metodo, "", tipoconexion);
                            }
                        ...
                        catch (Exception exc)
                        {
                            ...
                            Util.Bitacora_ComponenteSQL_EXECEP("", "INSERT MASIVO", tablaInsert, Usuario, tipoconexion, "", metodo, "", exc.ToString());
            ...
            catch (Exception e)
            {
                ...
                Util.Bitacora_ComponenteSQL_EXECEP("", "INSERT MASIVO", tablaInsert, Usuario, tipoconexion, "", metodo, "", e.ToString());
            }
        ...
        //        {
        //            banderaTransaccion = conexion.Executes_Mysql(xsql, mySqlSelectconex);//Ejecuta el borrado de la tabla  
        //        }
        ...
        //    return banderaTransaccion;
        //}
        public bool ValidarDevoluciones(string tipoconexion)
        {
            string mySqlSelectconex = "";
            try
            {
                ...
                while (!connectionSuccessful && connectionAttempt < 3)
                {
                    mySqlSelectconex = conexion.MYSQLSTRINGCONNECT(tipoconexion);
                    if (!string.IsNullOrEmpty(mySqlSelectconex) && !mySqlSelectconex.Contains("error"))
                    {
                ...
                {
                    bool executeStoreProcedure = conexion.Executes_Mysql("CALL CambiarEstadoDevolucionesAutomaticas();", mySqlSelectconex);
        ...
        public bool EnviarReporteDevolucionesInactivas(string tipoconexion)
        {
            const string queryDevoluciones = @"SELECT 
                                                    da.idRecno AS Identificador, 
                                                    date.descripcion AS Estado, 
                                                    tda.descripcion AS Tipo, 
                                                    fecha AS Fecha, 
                                                    contrato AS Contrato, 
                                                    CONCAT('₡', FORMAT(da.monto, 2)) AS `Monto`, 
                                                    CASE 
                                                        WHEN p.abrevia = '' THEN p.descripcio 
                                                        ELSE p.abrevia 
                                                    END AS `Medio de pago`,
                                                    autorizacion_tarjeta AS `Autorización de la tarjeta`,
                                                    informacion_tarjeta AS `Información de la tarjeta`,
                                                    fecha_registro AS `Fecha de registro`
                                                FROM 
                                                    devoluciones_automaticas AS da 
                                                INNER JOIN 
                                                    pago AS p ON da.medio_pago = p.idrecno 
                                                INNER JOIN 
                                                    tipo_devolucion_automatica AS tda ON tda.idRecno = da.fk_tipo_devolucion_automatica 
                                                INNER JOIN 
                                                    devoluciones_automaticas_tipo_estado date ON date.idRecno = da.fk_estado;";
            string mySqlSelectconex = "";
            try
            {
                int connectionAttempt = 0;
                ...
                if (connectionSuccessful)
                {
                    ...
                    DataTable resultQuery = MYSQL_C.MYSQLSelect(tipoconexion, queryDevoluciones);
                    if (resultQuery != null && resultQuery.Rows.Count > 0 && resultQuery.Columns.Count > 0)
                    {
                        using (MemoryStream excelStream = GenerarExcel(resultQuery))
                        {
                            return EnviarCorreoConAdjunto(excelStream, "ReporteDevolucionesInactivas.xlsx");
                        }
                    }
                }
            }
        ...
        private MemoryStream GenerarExcel(DataTable dataTable)
        {
            ...
            using (ExcelPackage package = new ExcelPackage())
            {
                ...
                using (var range = worksheet.Cells[1, 1, 1, dataTable.Columns.Count])
                {
                    ...
                    range.Style.Fill.BackgroundColor.SetColor(ColorTranslator.FromHtml("#D3D3D3"));
                }
        ...
        private bool EnviarCorreoConAdjunto(MemoryStream excelStream, string nombreArchivo)
        {
            MYSQL MYSQL_C = new MYSQL();
            try
            {
                DataTable parametros = MYSQL_C.MYSQLSelect("BILLINGMYSQL", "SELECT * FROM parametros_email WHERE estado = 'ACTIVO' AND CODIGO = '02'");
                DataTable correosDestinatarios = MYSQL_C.MYSQLSelect("BILLINGMYSQL", "SELECT emails FROM cuentas_correos_madrugada WHERE tipo = 76");

                if (parametros.Rows.Count > 0)
                {
                    string serverU = parametros.Rows[0]["server"].ToString();
                    int port = Convert.ToInt32(parametros.Rows[0]["puerto"].ToString());
                    string correoU = parametros.Rows[0]["username"].ToString();
                    string passwordU = parametros.Rows[0]["clave"].ToString();
                    var basicCredential = new NetworkCredential(correoU, passwordU);

                    MailMessage mail = new MailMessage();
                    SmtpClient SmtpServer = new SmtpClient(serverU);
                    SmtpServer.Credentials = basicCredential;

                    SmtpServer.Host = serverU;
                    SmtpServer.Port = port;

                    mail.From = new MailAddress(correoU, "TELECABLE");

                    // Agregar todas las direcciones de correo electrónico al destinatario
                    foreach (DataRow row in correosDestinatarios.Rows)
                    {
                        string correo = row["emails"].ToString().Trim();
                    ...
                    //mail.To.Add("erickvrojaspicado@gmail.com");
                    mail.Subject = "Reporte de devoluciones de dinero";
                    mail.Body = "Adjunto encontrarás el reporte de devoluciones inactivas.";
                    mail.IsBodyHtml = true;

                    Attachment attachment = new Attachment(excelStream, nombreArchivo, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
                ...
                Console.WriteLine("No existe información necesaria para enviar el correo");
                return false;
            }
            catch (Exception ex)
            {
                Console.WriteLine("Error al enviar correo con reporte de devolución: " + ex);
                return false;
            }
        }
