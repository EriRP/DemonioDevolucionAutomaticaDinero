...
using Operativa.ComponenteSQL.Conexion;
using System;
...
namespace Operativa.ComponenteSQL.Utiles
{
    public class Utiles
    {
        ...
        public Boolean Migracion_Data_MYSQL(Dictionary<string, object> Arr_INS_UPD, Trictionary<string, object, string[]> Arr_Where, string tipo_exec, string tabla, string metodo, string contrato)
        {
            ...
            ComponenteSQL.Conexion.MYSQL.MYSQL mysql = new ComponenteSQL.Conexion.MYSQL.MYSQL();
            //***********************FIN HEREDA*******************************
        ...
        public void Bitacora_ComponenteSQL(string usuario, string query, bool estado, string bd, string metodo, string contrato)
        {
            ...
            string select = "SELECT valor FROM parametros WHERE codigo = 160";
            DataTable dtParametros = mysql.MYSQLSelect("BILLINGMYSQL", select);
            string insertarBitacora = dtParametros.Rows[0][0].ToString();




            if (insertarBitacora == "1")
            {
                if (estado == false)
                {
                    Envia_Correo("Componente SQL: Se ha generado un error al realizar la sentencia: <font style='font-weight:bold;color:red'>" +
                    tipo_exec + " " + query + "</font> <p>De la Base de Datos: " + bd + "</p><p>Sistema de Origen: " + "CallCenter/" + metodo + "</p> <p>Por el usuario: " + usuario + "</p>", "Componente SQL: Error al ejecutar query");
                    estadoi = 0;
                }
                Arr_Insert.Clear();
                ...
                Arr_Insert.Add("origen", "CallCenter/" + metodo);//Sistema de Origen
        ...
        public void Bitacora_ComponenteSQL_EXECEP(string datos, string tipo, string tabla, string usuario, string bd, string where, string metodo, string contrato, string error)
        {
            ...
            if (insertarBitacora == "1")
            {
                ...
                tipo + " " + datos + "</font> <p>De la Base de Datos: " + bd + "</p><p>De la tabla: " + tabla + "</p><p>Sistema de Origen: " + "CallCenter/" + metodo + "</p> <p>Por el usuario: "
        ...
        public bool Envia_Correo(string cuerpo, string asunto)
        {
            ...
            try
            {
                ...
                MailAddress bcc2 = new MailAddress("gerardo.espinoza@telecablecr.com");
        ...
        //        {
        //            var tmp_tipo = pro.PropertyType.Name;
        //            if (tmp_tipo.Contains("DateTime"))
        //            {
        //                if (columnNames.Contains(pro.Name.ToLower().ToString().Trim()))
        //                {
        //                    //var fechas = Convert.ToDateTime("");
        //                    var fec = row[pro.Name].ToString().Trim();
        //                    if (columnNames.Contains(pro.Name.ToLower().ToString().Trim()) && !string.IsNullOrEmpty(fec))
        //                    {
        //                        try
        //                        {
        //                            pro.SetValue(objT, Convert.ToDateTime(fec));
        //                        }
        //                        catch (Exception)
        //                        {
        //                        }
